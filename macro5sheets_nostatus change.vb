...
Const SHEET_NAME_ATTENDANCE = "1930"
...
Sub ConvertToThreeSheets()
    ...
    sourceData = sourceWs.Range("A1:Y" & lastRow).Value
   ' Create the 3 sheets
    ...
    Call CreateSheet166(newWb, sourceData)
    Call CreateSheet1857(newWb, sourceData)
...
' Reorder sheets in correct sequence: 90291, 94010, 1857, 166, 1930
On Error Resume Next

newWb.Sheets(SHEET_NAME_ATTENDANCE).Move After:=newWb.Sheets(newWb.Sheets.Count)
newWb.Sheets("166").Move Before:=newWb.Sheets(SHEET_NAME_ATTENDANCE)
newWb.Sheets("1857").Move Before:=newWb.Sheets("166")
...
Sub CreateMiluimSheet(wb As Workbook, sourceData As Variant)
    ...
    ws.Cells(1, 17) = "Passport Number"       ' Q - passportNum
     ws.Cells(1, 18) = "Maamad"               ' R - maamad
      ws.Cells(1, 19) = "91220"               ' s - 91220
     ws.Cells(1, 20) = "total_paid"            ' t - total_paid
    
    
    ' Format headers
    With ws.Range("A1:D1,L1:T1")
        .Font.Bold = True
    ...
    For i = 2 To UBound(sourceData, 1)
        If Not IsEmpty(sourceData(i, 1)) And Not IsEmpty(sourceData(i, 2)) And sourceData(i, 24) = "RAW" Then
            rowCount = rowCount + 1
          ...
          ' ? החלף בבלוק הזה:
            ...
            If UBound(sourceData, 2) >= 19 Then
                ws.Cells(rowCount, 15) = sourceData(i, 18) ' S - Emergency Comp 4405
            End If
            If UBound(sourceData, 2) >= 20 Then
                ws.Cells(rowCount, 16) = sourceData(i, 19)  ' T - Emergency 40%
            End If
            If UBound(sourceData, 2) >= 21 Then
                ws.Cells(rowCount, 17) = sourceData(i, 20)  ' U - Passport Number
            End If
            ' Maamad logic: if column 21 has value → 4410, else → 1309
If UBound(sourceData, 2) >= 22 Then
    If Not IsEmpty(sourceData(i, 21)) And sourceData(i, 21) <> "" Then
        ws.Cells(rowCount, 18) = "4410"
    Else
        ws.Cells(rowCount, 18) = "1309"
    End If
End If
  If UBound(sourceData, 2) >= 23 Then
                ws.Cells(rowCount, 19) = sourceData(i, 22)  ' s - 91220
            End If

            If UBound(sourceData, 2) >= 24 Then
                ws.Cells(rowCount, 20) = sourceData(i, 23)  ' t - totalpaid
            End If
        End If
    Next i
...
' Creates attendance data sheet (1930) with no duplicates
...
' Creates sheet 166 for SHEET_NAME = "166"
Sub CreateSheet166(wb As Workbook, sourceData As Variant)
    Dim ws As Worksheet
    Dim rowCount As Long, i As Long
    ...
    ' Check if there's any data with SHEET_NAME = "166"
    For i = 2 To UBound(sourceData, 1)
        If Not IsEmpty(sourceData(i, 1)) And sourceData(i, 24) = "166" Then
            hasData = True
            Exit For
        End If
    Next i
    
    ' Only create sheet if data exists
    ...
    ' Process data for SHEET_NAME = "166"
    For i = 2 To UBound(sourceData, 1)
        If Not IsEmpty(sourceData(i, 1)) And sourceData(i, 24) = "166" Then
            rowCount = rowCount + 1
            ws.Cells(rowCount, 1) = sourceData(i, 1)   ' ID_NUM (column 1)
            ws.Cells(rowCount, 2) = sourceData(i, 2)   ' DATE_FROM (column 2)
            ws.Cells(rowCount, 3) = sourceData(i, 18)  ' Value from column 21
        End If
    Next i
    ...
    ' Sort by employee ID
    If rowCount > 1 Then
        ws.Range("A1:C" & rowCount).Sort Key1:=ws.Range("A1"), Order1:=xlAscending, Header:=xlYes
    End If
End Sub
' Creates sheet 1857 for SHEET_NAME = "1857"
Sub CreateSheet1857(wb As Workbook, sourceData As Variant)
    Dim ws As Worksheet
    Dim rowCount As Long, i As Long
    Dim hasData As Boolean
    ...
    ' Check if there's any data with SHEET_NAME = "1857"
    For i = 2 To UBound(sourceData, 1)
        If Not IsEmpty(sourceData(i, 1)) And sourceData(i, 24) = "1857" Then
            hasData = True
            Exit For
        End If
    Next i
    ...
    ' Process data for SHEET_NAME = "1857"
    For i = 2 To UBound(sourceData, 1)
        If Not IsEmpty(sourceData(i, 1)) And sourceData(i, 24) = "1857" Then
            rowCount = rowCount + 1
            ws.Cells(rowCount, 1) = sourceData(i, 1)   ' ID_NUM (column 1)
